...
param_default <- c(0.4, 1.0, 0.4, 0.2, 0.2) # PBD parameters 
param_n <- 5 # Number of values tested
param_range <- c(10^-1, 10^-0.5, 1.0, 10^0.15, 10^0.30)

age <- 15 # Duration simulations

i_tree <- as.numeric(args_input[1])
print(i_tree)
set.seed(23 + 3 * i_tree)


# Simulation and inference 
df_simul_infer <- as.data.frame(t(sapply(1:param_n, function(i_param_var) {
  param <- param_default
  param[1] <- param_range[i_param_var] * param_default[1]
  param[3] <- param[1]
  
  fname <- paste0("-var", i_param_var, "-rep", i_tree)
