...
keys_keep = ['i_param_var'] + par_names_PBD

list_df = []


for i_param_var in range(1, 1+n_param_var):
    par_simul = (df.loc[(df.i_param_var == i_param_var) & (df.replicate == 1)]).to_dict("records")[0]
    df_simul = df.loc[(df.i_param_var == i_param_var)]
    ...
    for i_tree in range(n_replicates):
        ...
        if sr > 2:
            fname = "mcmc-var{}-rep{}.csv".format(i_param_var, i_tree)
    ...
    for name, data in zip(rate_names, rate_data):
        ...
        with open(outdir + "/boxplot-"+name+"-var{}.json".format(i_param_var), 'w') as f: 
            json.dump(box, f)
