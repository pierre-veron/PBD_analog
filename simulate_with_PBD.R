...
n_tree <- 20 # Number of replicates for each combination of parameter
...
simul_infer <- lapply(1:length(param_default), function(i_param) {
  ...
  list_df <- lapply(1:param_n, function(i_param_var) {
    
    param[i_param] <- param_ranges[[i_param]][i_param_var] * param_default[i_param]
    
    df <- as.data.frame(t(sapply(1:n_tree, function(i_tree) {
...
# save results
write.csv(df_simul_infer, file = paste0(outdir, "/all_simulations_inference.csv"))