...
manual_trans <- scales::trans_new("tanh", transform = function(x) tanh(x*1.5), inverse = function(y) atanh(y)/1.5)
...
create_plot <- function(data, y_var, x_label, y_label, eq_var) {
  ...
  filtered_data <- data[data[y_var]<100,]
  
  ggplot(filtered_data, aes(x = split_times, y = !!sym(y_var))) +
    #geom_jitter(alpha = 0.03, col = brewer.pal(n = 7, name = "YlOrBr")[i_var + 2]) +
    geom_line(aes(group = replicate), alpha = 0.1, col = brewer.pal(n = 7, name = "YlOrBr")[i_var + 2], show.legend = FALSE) +
    geom_hline(aes(yintercept = !!sym(eq_var))) +
    geom_line(data = median_data, aes(x = split_times, y = median_value), linetype = "dotdash", color = "blue4", lwd=1) +  # Add the median trend line
    scale_x_continuous(trans = "pseudo_log") +
    scale_y_continuous(transform = manual_trans, breaks = c(0,0.1,0.2,0.3,0.5,1,5), limits = c(0,NA)) +
    labs(title = plot_title, x = x_label, y = y_label) + 
    theme_tufte() +
    theme(
      plot.margin = margin(0, 5, 0, 5),
...
for (i_var in unique_i_param_var) {
  for (param_var in unique_param_vary) {
    ...
    y_label_lambda <- if (param_var == 1 & i_var == 3) "Inferred Birth Rate" else ""
    y_label_mu <- if (param_var == 1 & i_var == 3) "Inferred Death Rate" else ""
