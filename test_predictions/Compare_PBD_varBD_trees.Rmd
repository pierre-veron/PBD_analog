...
```{r}
summary(sapply(sim_BDtrees, sapply, function(x)ifelse(is.phylo(x), is.binary(x), NA)))
...
```{r}
trees_B2 <- sapply(sim_BDtrees_binary, sapply, function(result){
  if (is.numeric(result)){
    return(NaN)
  }else{
    return(treebalance::B2I(result))
  }
  })
trees_B2
```
