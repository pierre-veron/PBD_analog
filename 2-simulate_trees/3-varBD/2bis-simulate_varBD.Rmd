...
```{r}
np <- import("numpy")
BD_rtt <- np$load("../../simulations_output/3-varBD/variable_BDrates_simplified_PBD.npy", allow_pickle=T)
BD_rtt[[1]]
```
...
for (i in seq_along(sim_BDtrees_binary)) {
  ...
  if (any(sim_success)){
    write.tree(sim_BDtrees_binary[[i]][sim_success], file=paste0("../../simulations_output/3-varBD/trees_simplified/sim_varBD_simplified_PBD_pars", i, ".trees"))
  }
}
```
...
reload_sim_BDtrees <- function(path, num_pars, n_trees) {
  ...
  for (i in seq_len(num_pars)) {
    file_path <- file.path(path, paste0("sim_varBD_simplified_PBD_pars", i, ".trees"))
...
sim_BDtrees_binary <- reload_sim_BDtrees("../../simulations_output/3-varBD/trees_simplified", nrow(BD_rtt), n_trees)
```
...
```{r}
simul_infer <- read.csv("../../simulations_output/1-PBD/all_simulations_inference_simplified_PBD.csv", row.names = 1)
...
trees_stats
write.csv(trees_stats, "../../simulations_output/3-varBD/simulated_varBD_trees_stats_simplified_PBD.csv")
```
